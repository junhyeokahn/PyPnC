...
def setUpWorld(initialSimSteps=100):
    ...
    # Load plane
    p.loadURDF(cwd + "/../pybullet_robots/data/plane.urdf", [0, 0, -1],
               useFixedBase=True)
    ...
    baxterId = p.loadURDF(
        cwd +
        "/../pybullet_robots/data/baxter_common/baxter_description/urdf/toms_baxter.urdf",
        useFixedBase=True)
    p.resetBasePositionAndOrientation(baxterId, [0.5, -0.8, 0.0],
                                      [0., 0., -1., -1.])
...
def accurateIK(bodyId,
    ...
    while (not closeEnough and iter < maxIter):
        if useNullSpace:
            jointPoses = p.calculateInverseKinematics(bodyId,
                                                      endEffectorId,
                                                      targetPosition,
            ...
            jointPoses = p.calculateInverseKinematics(bodyId, endEffectorId,
                                                      targetPosition)
        ...
        newPos = ls[4]
        diff = [
            targetPosition[0] - newPos[0], targetPosition[1] - newPos[1],
...
if __name__ == "__main__":
    ...
    for _ in range(maxIters):
        ...
        jointPoses = accurateIK(baxterId,
                                endEffectorId,
                                targetPosition,
                                lowerLimits,
                                upperLimits,
