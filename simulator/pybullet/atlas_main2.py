...
atlas = p.loadURDF(
    cwd + "/../pybullet_robots/data/atlas/atlas_v4_with_multisense.urdf",
    [-2, 3, -0.5])
for i in range(p.getNumJoints(atlas)):
    p.setJointMotorControl2(atlas, i, p.POSITION_CONTROL, 0)
    # p.setJointMotorControl2(atlas, i, p.VELOCITY_CONTROL, force=2)
    print(p.getJointInfo(atlas, i))

if 1:
    objs = p.loadSDF(cwd + "/../pybullet_robots/data/botlab/botlab.sdf",
                     globalScaling=2.0)
    zero = [0, 0, 0]
    ...
    p.loadURDF(cwd + "/../pybullet_robots/data/plane.urdf", [0, 0, -3])

p.loadURDF(cwd + "/../pybullet_robots/data/boston_box.urdf", [-2, 3, -2],
           useFixedBase=True)
...
p.loadURDF(cwd + "/../pybullet_robots/data/boston_box.urdf", [0, 3, -2],
           useFixedBase=True)
