...
int main() {
  YAML::Node cfg = YAML::LoadFile(THIS_COM "config/towr_plus/atlas_block.yaml");
  Clock clock = Clock();
  double time_solving(0.);

  // Locomotion Task
  LocomotionTask task = LocomotionTask("atlas_block");
  task.from_yaml(cfg["locomotion_task"]);

  // Locomotion Solution
  LocomotionSolution sol =
      LocomotionSolution("atlas_block", cfg["locomotion_param"]);

  // Construct NLP from locomotion task
  NlpFormulation formulation;

  formulation.model_ = RobotModel(RobotModel::Atlas);
  Eigen::Vector3d max_dev(0.18, 0.1, 0.15);
  Eigen::Vector3d min_dev(-0.18, -0.1, -0.1);
