...
int main() {
  YAML::Node cfg =
      YAML::LoadFile(THIS_COM "config/towr_plus/nao_forward_walk.yaml");
  Clock clock = Clock();
  double time_solving(0.);

  // Locomotion Task
  LocomotionTask task = LocomotionTask("nao_forward_walk");
  task.from_yaml(cfg["locomotion_task"]);

  // Locomotion Solution
  LocomotionSolution sol =
      LocomotionSolution("nao_forward_walk", cfg["locomotion_param"]);

  // Construct NLP from locomotion task
  NlpFormulation formulation;
  formulation.rom_buf = 0.01;
  formulation.model_ = RobotModel(RobotModel::Nao);
