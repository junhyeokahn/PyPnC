...
public:
  ...
  double t_transfer = 0.1; // 0.125 ; //0.1; // exponential interpolation
                           // transfer time during initial transfer or same
                           // step transfer
  double t_ds = 0.05;      // double support polynomial transfer time
  double t_ss = 0.3;       // single support exponential interpolation  time
  double percentage_settle =
  ...
  // input: input_footstep_list - a list of footsteps to take not including the
  ...
  // current stance configuration.
  //        initial_footstance  - a footstep object describing the stance leg.
  //        initial_rvrp        - an initial virtual repelant point (eg: average
  //        of the stance feet's rvrp).
  void
  initialize_footsteps_rvrp(const std::vector<Footstep> &input_footstep_list,
                            const Footstep &initial_footstance,
  ...
  // input: input_footstep_list - a list of footsteps to take not including the
  // current stance configuration.
  //        left_footstance        - a footstep object describing the left
  //        stance feet
  //        right_footstance       - a footstep object describing the right
  //        stance feet
  void
  initialize_footsteps_rvrp(const std::vector<Footstep> &input_footstep_list,
  ...
  // computes the CoM velocity given the current CoM position and DCM velocity
  // state.
  ...
  // returns the double support starting and ending time of the step_index from
  // t_start.
  double get_double_support_t_start(const int step_index);
  double get_double_support_t_end(const int step_index);

  // if the step_index is a swing type, returns true an populates the swing
  // start time and end
...
private:
  ...
  // computes all the dcm states. Computation properly populates the
  ...
  // computes the step i's initial DCM state and the end-of-step i-1's dcm
  // state.
  ...
  // Computes the double support DCM boundary conditions for continuous double
  // support trajectories
  // step_index the boundary condition for the step index
  // t_DS_{ini, end} the interpolation duration away from a Virtual repelant
  // point.
  ...
  // Returns the DCM velocity exponential interpolation for the requested
  // step_index.
  // time, t, is clamped between 0.0 and t_step.
  Eigen::Vector3d get_DCM_vel_exp(const int &step_index, const double &t);

  // Returns the DCM acceleration exponential interpolation for the requested
  // step_index.
  // time, t, is clamped between 0.0 and t_step.
